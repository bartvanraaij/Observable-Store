...
var ObservableStore = /** @class */ (function () {
    ...
    ObservableStore.prototype.setState = function (state, action, dispatchState) {
        ...
        if (dispatchState) {
            this._dispatchState(state, action);
        }
    ...
    ObservableStore.prototype._dispatchState = function (stateChanges, action) {
        var stateOrSlice = this._getStateOrSlice(storeState);
        ...
        if (this._settings.includeActionOnSubscribe) {
            if (this._settings.includeStateChangesOnSubscribe) {
                this._stateDispatcher.next({ action: action, state: clonedStateOrSlice, stateChanges: stateChanges });
                globalStateDispatcher.next({ action: action, state: clonedGlobalState, stateChanges: stateChanges });
            }
            else {
                this._stateDispatcher.next({ action: action, state: clonedStateOrSlice });
                globalStateDispatcher.next({ action: action, state: clonedGlobalState });
            }
        }
        else {
            if (this._settings.includeStateChangesOnSubscribe) {
